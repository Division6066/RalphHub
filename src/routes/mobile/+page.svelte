<script lang="ts">
	...
	onMount(async () => {
		...
		loading = false;
		// Mobile sync backend is planned for v1.1 — show setup guide
	});
	...
	async function enableSync() {
		...
		enabling = true;
		try {
			// Mobile sync backend planned for v1.1 — show instructions
			syncStatus = { enabled: false, port: 7474, qrData: '', localIp: '192.168.1.x', connectedDevices: 0 };
			statusMsg = 'Mobile sync setup: Download the AmitOS APK and connect to this machine on the same network.';
